...
def patch_ex(
    ...
    resize=True,
    gamma_params=None,
    intensity_logistic_params=(1 / 6, 20),
    ...
    if skip_background is not None and not cutpaste_patch_generation:
        ...
        for background, threshold in skip_background:
            src_object_mask &= np.uint8(
                np.abs(ima_src.mean(axis=-1, keepdims=True) - background) > threshold
            )
            dest_object_mask &= np.uint8(
                np.abs(ima_dest.mean(axis=-1, keepdims=True) - background) > threshold
            )
        src_object_mask[..., 0] = cv2.medianBlur(
            src_object_mask[..., 0], 7
        )  # remove grain from threshold choice
        dest_object_mask[..., 0] = cv2.medianBlur(
            dest_object_mask[..., 0], 7
        )  # remove grain from threshold choice
    else:
    ...
    coor_min_dim1, coor_max_dim1, coor_min_dim2, coor_max_dim2 = (
        mask.shape[0] - 1,
        0,
        mask.shape[1] - 1,
        0,
    ...
    for i in range(num_patches):
        if i == 0 or np.random.randint(2) > 0:  # at least one patch
            (
                patchex,
                ((_coor_min_dim1, _coor_max_dim1), (_coor_min_dim2, _coor_max_dim2)),
                patch_mask,
            ) = _patch_ex(
                patchex,
                ima_src,
            ...
            if patch_mask is not None:
                mask[_coor_min_dim1:_coor_max_dim1, _coor_min_dim2:_coor_max_dim2] = (
                    patch_mask
                )
    ...
    label_mask = np.uint8(
        np.mean(
            np.abs(1.0 * mask * ima_dest - 1.0 * mask * patchex), axis=-1, keepdims=True
        )
        > tol
    )
    label_mask[..., 0] = cv2.medianBlur(
        label_mask[..., 0], 5
    )  # remove grain from threshold choice
...
def _patch_ex(
    ...
    if cutpaste_patch_generation:
        ...
        patch_width_dim1 = int(
            np.rint(
                np.clip(np.sqrt(area_ratio * aspect_ratio * dims[0] ** 2), 0, dims[0])
            )
        )
        patch_width_dim2 = int(
            np.rint(np.clip(area_ratio * dims[0] ** 2 / patch_width_dim1, 0, dims[1]))
        )
        #  3. sample location such that patch is contained entirely within the image
        ...
        patch_mask = np.ones(
            (coor_max_dim1 - coor_min_dim1, coor_max_dim2 - coor_min_dim2, 1),
            dtype=np.uint8,
        )
    else:
        skip_background = (src_object_mask is not None) and (
            dest_object_mask is not None
        )
        dims = np.array(ima_dest.shape)
        ...
        if gamma_params is not None:
            ...
            patch_width_dim1 = int(
                np.clip(
                    (lower_bound + np.random.gamma(shape, scale)) * dims[0],
                    min_width_dim1,
                    max_width_dim1,
                )
            )
            patch_width_dim2 = int(
                np.clip(
                    (lower_bound + np.random.gamma(shape, scale)) * dims[1],
                    min_width_dim2,
                    max_width_dim2,
                )
            )
        ...
        while not found_patch:
            ...
            if num_ellipses is not None:
                ...
                patch_mask = np.zeros(
                    (coor_max_dim1 - coor_min_dim1, coor_max_dim2 - coor_min_dim2),
                    dtype=np.uint8,
                )
                ...
                for _ in range(num_ellipses):
                    ...
                    ellipse = (
                        ((x - x0) * np.cos(theta) + (y - y0) * np.sin(theta)) / a
                    ) ** 2 + (
                        ((x - x0) * np.sin(theta) + (y - y0) * np.cos(theta)) / b
    ...
    if resize:
        ...
        new_width = np.clip(
            int(new_height / height * width), min_width_dim2, max_width_dim2
        )
        new_height = np.clip(
            int(new_width / width * height), min_width_dim1, max_width_dim1
        )  # in case there was clipping
    ...
    if skip_background:
        src_object_mask = cv2.resize(
            src_object_mask[
                coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2, 0
            ],
            (width, height),
        )
    ...
    if shift:
        ...
        while not found_center:
            center_dim1 = np.random.randint(
                height // 2 + 1, ima_dest.shape[0] - height // 2 - 1
            )
            center_dim2 = np.random.randint(
                width // 2 + 1, ima_dest.shape[1] - width // 2 - 1
            )
            coor_min_dim1, coor_max_dim1 = (
                center_dim1 - height // 2,
                center_dim1 + (height + 1) // 2,
            )
            coor_min_dim2, coor_max_dim2 = (
                center_dim2 - width // 2,
                center_dim2 + (width + 1) // 2,
            )
            ...
            if skip_background:
                ...
                found_center = (
                    np.sum(src_object_mask)
                    / (patch_mask.shape[0] * patch_mask.shape[1])
                    > min_object_pct  # contains object
                    and np.sum(src_and_dest) / np.sum(src_object_mask) > min_overlap_pct
                )  # object overlaps src object
            else:
                found_center = True
            attempts += 1
            if attempts == 200:
    ...
    if skip_background:
        patch_mask &= (
            src_object_mask
            | dest_object_mask[coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2]
        )

    if mode == "swap":
        patchex = ima_dest.copy()
        before = patchex[coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2]
        patchex[coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2] -= (
            patch_mask * before
        )
        patchex[coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2] += (
            patch_mask * src
        ...
        before = patchex[coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2]
        patchex[coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2] -= (
            factor * patch_mask * before
        )
        patchex[coor_min_dim1:coor_max_dim1, coor_min_dim2:coor_max_dim2] += (
            factor * patch_mask * src
        )
        ...
        try:
            ...
            if ima_dest.shape[2] == 1:  # grayscale
                ...
                src_3 = np.concatenate(
                    (src, np.zeros_like(src), np.zeros_like(src)), axis=2
                )
                ima_dest_3 = np.concatenate(
                    (ima_dest, np.zeros_like(ima_dest), np.zeros_like(ima_dest)), axis=2
                )
                patchex = cv2.seamlessClone(
                    src_3, ima_dest_3, patch_mask_scaled, center, mode
                )
                patchex = patchex[..., 0:1]  # extract first channel
            else:  # RGB
                patchex = cv2.seamlessClone(
                    src, ima_dest, patch_mask_scaled, center, mode
                )
