...
class SelfSupMVTecDataset(Dataset):
    def __init__(
        ...
        self.anomaly_gpt_configs = anomaly_gpt_configs
    ...
    def __getitem__(self, idx):
        ...
        s.choi
        get
        - if train: (only images, labels exist)
            images, masks = nsa(augment_transform(images)))
            return images, masks, labels
        - if test: (images, labels, masks exist)
            images = augment_transform(images)
            masks = augment_transform(masks)
            return images, masks, labels
        """
        ...
        if self.self_sup:
            ...
            if self.transform is not None:
                mask = self.transform(
                    mask
                )  # tranform should be deterministic when testing
    ...
    def load_dataset_folder(self, low_res):
        """
        s.choi
        - load image from root_folders
        - if train, (mvtec only contain good images in train)
        ...
        """
        s.choi
        since there are no masks in train set, this is only for test.
        """
        ...
        for path in mask_paths:
            if path is None:
                ...
                import pdb; pdb.set_trace(); # to see whether what was before applying to.tensor
